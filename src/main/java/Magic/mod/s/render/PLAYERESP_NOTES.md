...
контур всегда сверху, и вопрос лишь в том, что попало в силуэт. Ваниль
пишет туда только голую модель (в ветке `renderOutlines` у `doRender()`
`renderLayers()` не вызывается вовсе) — именно поэтому контур по умолчанию
идёт сквозь броню. Значит **вкл** это нетронутая ваниль, и работу делает
только **выкл**: тогда в тот же буфер дорисовываются layers, броня входит
в силуэт, и контур обходит её вместо того, чтобы пересекать.
`setScoreTeamColor()` к этому моменту уже выключил обе текстурные единицы
и выставил плоский цвет, так что layers ложатся именно силуэтом.

`renderLayers()` требует `partialTicks`, а это параметр `doRender()`, и в
точке перезаписи `renderModel()` он недоступен (в классе нет
LocalVariableTable, так что javassist не возьмёт его по имени). Берётся
`mc.timer.renderPartialTicks` — это не приближение, а то же самое число:
`runGameLoop()` рендерит через
`entityRenderer.func_181560_a(this.timer.renderPartialTicks, …)` и поле в
течение этого вызова не переписывается.
