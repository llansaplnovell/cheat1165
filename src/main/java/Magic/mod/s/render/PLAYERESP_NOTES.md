...
Поэтому `renderOutlineArmor()` рисует **только** слои `LayerArmorBase` и
принудительно ставит состояние так, чтобы в буфер шла одна непрозрачная
альфа: blend выключен (glint у зачарованной брони блендит аддитивно),
alpha-тест выключен, обе текстурные единицы выключены. Цвет отдан слою —
шейдер смотрит на альфу, а не на цвет, да и ванильный team color всё
равно белый. После отрисовки состояние возвращается через `GlStateManager`
(чтобы его кэш не разъехался с реальным GL): glint внутри
`LayerArmorBase` заканчивает на своих значениях depth (`GL_LEQUAL`,
`depthMask` true), а проходу ещё рисовать другие сущности — с
`GL_ALWAYS`.
