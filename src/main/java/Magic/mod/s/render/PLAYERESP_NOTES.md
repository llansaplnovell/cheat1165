...
Outline или Minecraft (`ValuePanel` уважает `Value.isOpen()`).

- **выкл (по умолчанию)** — броня перекрывает/формирует контур
- **вкл** — контур просвечивает сквозь броню
...
Броня это layer, который рисуется позже и с включённым depth-тестом,
поэтому закрашивает контур; отсюда и то, что по умолчанию контур сквозь
броню не виден. Если отрисовать layers **до** stencil-блока, контур
ложится поверх брони. (Это заодно тот вызов `renderLayers()`, что стоит у
Peter в начале его stencil-блока и который в первой версии переноса был
пропущен.)

**Minecraft.** Тут outline-буфер накладывается на готовый кадр, то есть
контур и так всегда сверху; вопрос лишь в том, входит ли броня в силуэт.
Ваниль пишет в этот буфер только голую модель, поэтому контур обводит
тело и проходит по броне — то есть «сквозь броню» это его поведение по
умолчанию. Дорисовка layers в тот же буфер заставляет контур обходить
броню. `setScoreTeamColor()` при этом держит текстуры выключенными и
плоский цвет, так что броня попадает именно как силуэт.

`renderLayers()` требует `partialTicks` — это параметр `doRender()`, и в
точке перезаписи `renderModel()` он не в области видимости, поэтому патч
дополнительно сохраняет его через `insertBefore`.
