...
только **выкл**: тогда в тот же буфер дорисовывается броня, она входит
в силуэт, и контур обходит её вместо того, чтобы пересекать.

### Почему именно броня, а не `renderLayers()`

Первая версия звала `renderLayers()` — и режим стал закрашивать игрока
свечением целиком. Причина в `magic_esp_edge.fsh`: он ищет края по
**альфе** соседних текселей. Силуэт читается как контур только пока альфа
внутри него однородна — одна плоская область на очищенном буфере, края
только по её границе. `LayerHeldItem` и `LayerCustomHead` идут через
`RenderItem`, а тот **включает текстурирование обратно** (его выключил
`setScoreTeamColor()`), и текстурированная отрисовка пишет альфу
текстуры — дырки и мягкие края по всей фигуре, и шейдер честно обводит
каждый из них. Плюс предмет в руке сам становился частью силуэта, что уже
не «тот же контур, просто не сквозь броню».

Поэтому `renderOutlineArmor()` рисует **только** слои `LayerArmorBase`.

### Почему выключить текстуры недостаточно

Второй заход выключал текстуры и blend и отдавал цвет слою — и в игре
контур стал **фиолетовым** и лёг поверх брони. Фиолетовый выдал причину:
`LayerArmorBase` сам ставит `glColor` на каждый кусок (цвет краски у
кожаной брони), а для зачарованной поверх той же модели рисует glint —
`color(0.5, 0.25, 0.8)` с аддитивным блендом. То есть слой сам решает,
каким цветом и с какой альфой рисовать, и запретить ему это снаружи
нельзя.

Решение — сделать его решение неважным: текстурирование **включено**, но
texture environment выставлен в `GL_REPLACE` из `GL_CONSTANT` и для RGB,
и для альфы. Тогда каждый фрагмент выходит ровно константой, независимо
от того, какая текстура привязана, какой `glColor` выставил слой и что
поверх сблендил glint (он рисует тем же цветом, что и закрашивает). Это
ровно тот механизм, который Mojang добавили в 1.9 как
`GlStateManager.enableOutlineMode()` под эту же задачу.

Константа читается из `GL_CURRENT_COLOR` — там всё ещё цвет, который
поставил `setScoreTeamColor()` и которым нарисовано тело, так что броня и
тело ложатся в буфер одной областью, а не двумя.

Состояние трогается сырым GL, а не `GlStateManager`: здесь режим отказа —
это именно no-op (клиент много где ходит в GL напрямую, и кэш уже может
расходиться с драйвером — ровно для этого существует
`forceOutlineBlend`). Что меняется, то сначала читается через
`glIsEnabled` и возвращается как было; texture environment
восстанавливается кодом самого MC из `unsetBrightness()`. Depth, наоборот,
возвращается через `GlStateManager`: glint внутри `LayerArmorBase` тоже
меняет его через `GlStateManager` (`GL_EQUAL`/`GL_LEQUAL`, `depthMask`),
так что кэш к этому моменту уверен в `GL_LEQUAL`, а проходу ещё рисовать
другие сущности с `GL_ALWAYS` — поставь я это сырым GL, следующий
`GlStateManager.depthFunc()` стал бы тем самым no-op.
